...
MessiahEncoder::MessiahEncoder()
    : pos_att_(nullptr),
      normal_att_(nullptr),
      color_att_(nullptr),
      tex1_coord_att_(nullptr),
      tex2_coord_att_(nullptr),
      tangent_att_(nullptr),
      binormal_att_(nullptr),
      weight_att_(nullptr),
      indices_att_(nullptr),
      indices_name_att_(nullptr),
...
bool MessiahEncoder::EncodeToFile(const PointCloud &pc,
...
bool MessiahEncoder::EncodeToFile(const Mesh &mesh, const std::string &file_name) {
  in_mesh_ = &mesh;
  return EncodeToFile(static_cast<const PointCloud &>(mesh), file_name);
}

bool MessiahEncoder::EncodeToBuffer(const PointCloud &pc,
...
bool MessiahEncoder::EncodeToBuffer(const Mesh &mesh, EncoderBuffer *out_buffer) {
  in_mesh_ = &mesh;
  return EncodeToBuffer(static_cast<const PointCloud &>(mesh), out_buffer);
}

bool MessiahEncoder::EncodeInternal() {
  pos_att_ = nullptr;
  normal_att_ = nullptr;
  added_edges_att_ = nullptr;
  current_sub_obj_id_ = -1;
...
bool MessiahEncoder::ExitAndCleanup(bool return_value) {
  in_mesh_ = nullptr;
  in_point_cloud_ = nullptr;
  out_buffer_ = nullptr;
  pos_att_ = nullptr;
...
bool MessiahEncoder::GetAddedEdges() {
...
bool MessiahEncoder::GetSubObjects() {
...
bool MessiahEncoder::EncodeMaterialFileName() {
  const GeometryMetadata *pc_metadata = in_point_cloud_->GetMetadata();
...
bool MessiahEncoder::EncodePositions() {
...
bool MessiahEncoder::EncodeTextureCoordinates() {
  const PointAttribute *const att =
...
bool MessiahEncoder::EncodeNormals() {
  const PointAttribute *const att =
...
bool MessiahEncoder::EncodeFaces() {
...
bool MessiahEncoder::EncodePolygonalFaces() {
...
bool MessiahEncoder::EncodeFaceAttributes(FaceIndex face_id) {
  if (sub_obj_att_) {
...
bool MessiahEncoder::EncodeMaterial(FaceIndex face_id) {
  int material_id = 0;
...
bool MessiahEncoder::EncodeSubObject(FaceIndex face_id) {
  int sub_obj_id = 0;
  // Pick the first corner, all corners of a face should have same id.
  const PointIndex vert_index = in_mesh_->face(face_id)[0];
...
bool MessiahEncoder::EncodeFaceCorner(FaceIndex face_id, int local_corner_id) {
  const PointIndex vert_index = in_mesh_->face(face_id)[local_corner_id];
  return EncodeFaceCorner(vert_index);
}

bool MessiahEncoder::EncodeFaceCorner(PointIndex vert_index) {
  buffer()->Encode(' ');
...
void MessiahEncoder::EncodeFloat(float val) {
...
void MessiahEncoder::EncodeFloatList(float *vals, int num_vals) {
...
void MessiahEncoder::EncodeInt(int32_t val) {
  snprintf(num_buffer_, sizeof(num_buffer_), "%d", val);
  buffer()->Encode(num_buffer_, strlen(num_buffer_));
}

bool MessiahEncoder::IsNewEdge(const CornerTable &ct, CornerIndex ci) const {
...
void MessiahEncoder::FindOriginalFaceEdges(FaceIndex face_index,
