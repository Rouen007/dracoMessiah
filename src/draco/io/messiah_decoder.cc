...
Status MessiahDecoder::DecodeFromFile(const std::string &file_name,
                                  Mesh *out_mesh) {
  return DecodeFromFile(file_name, out_mesh, nullptr);
}

Status MessiahDecoder::DecodeFromFile(const std::string &file_name, Mesh *out_mesh,
...
Status MessiahDecoder::DecodeFromBuffer(DecoderBuffer *buffer, Mesh *out_mesh) {
  out_mesh_ = out_mesh;
  return DecodeFromBuffer(buffer, static_cast<PointCloud *>(out_mesh));
}

Status MessiahDecoder::DecodeFromBuffer(DecoderBuffer *buffer,
                                    PointCloud *out_point_cloud) {
  out_point_cloud_ = out_point_cloud;
...
Status MessiahDecoder::DecodeInternal() {
...
void MessiahDecoder::ResetCounters() {
  num_obj_faces_ = 0;
  num_positions_ = 0;
  num_tex_coords_ = 0;
  num_normals_ = 0;
...
bool MessiahDecoder::ParseDefinition(Status *status) {
  char c;
...
bool MessiahDecoder::ParseVertexPosition(Status *status) {
  std::array<char, 2> c;
  if (!buffer()->Peek(&c)) {
    return false;
  }
...
bool MessiahDecoder::ParseNormal(Status *status) {
  std::array<char, 2> c;
  if (!buffer()->Peek(&c)) {
    return false;
  }
...
bool MessiahDecoder::ParseTexCoord(Status *status) {
  std::array<char, 2> c;
  if (!buffer()->Peek(&c)) {
    return false;
  }
...
bool MessiahDecoder::ParseFace(Status *status) {
...
bool MessiahDecoder::ParseMaterialLib(Status *status) {
...
bool MessiahDecoder::ParseMaterial(Status * /* status */) {
...
bool MessiahDecoder::ParseObject(Status *status) {
  std::array<char, 2> c;
  if (!buffer()->Peek(&c)) {
    return false;
  }
...
bool MessiahDecoder::ParseVertexIndices(std::array<int32_t, 3> *out_indices) {
...
void MessiahDecoder::MapPointToVertexIndices(
...
bool MessiahDecoder::ParseMaterialFile(const std::string &file_name,
                                   Status *status) {
...
bool MessiahDecoder::ParseMaterialFileDefinition(Status * /* status */) {
  char c;
  parser::SkipWhitespace(buffer());
  if (!buffer()->Peek(&c)) {
    // End of file reached?.
...
//          A           E
//
inline int MessiahDecoder::Triangulate(int tri_index, int tri_corner) {
  return tri_corner == 0 ? 0 : tri_index + tri_corner;
}

inline bool MessiahDecoder::IsNewEdge(int tri_count, int tri_index,
                                  int tri_corner) {
